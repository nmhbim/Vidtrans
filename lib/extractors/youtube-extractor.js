...
class YouTubeExtractor extends BaseExtractor {
  ...
  async _fetchSubtitle(enrichedUrl) {
    ...
    // 3. Group sentences into longer chunks (~20-25s) for the listener
    // Strategy: Make chunks sequential and gapless to ensure stable display.
    const chunks = [];
    const TARGET_CHUNK_MS = 22000;
    let startIndex = 0;
    ...
    const finalEvents = chunks.map((chunk, i) => {
      ...
      return {
        start: chunk.start,
        duration: Math.max(duration, 5000), // Minimum 5s to ensure visibility
        text: chunk.text
      };
    });
